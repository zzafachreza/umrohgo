...
export default function JamaahAgen({ navigation, route }) {
    ...
    const simpan = () => {
        ...
        setLoading(true);
        axios.post(apiURL + 'register_jamaah', data).then(res => {
        ...
        <ImageBackground source={require('../../assets/bgone.png')} style={styles.background}>
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : ''} style={{ flex: 1 }}>
                <ScrollView
                    ...
                    <Text style={styles.title}>DAFTARKAN JAMAAH JADI AGEN</Text>
                    <Text style={styles.subtitle}>Silahkan daftarkan jamaah kamu menjadi agen supaya kamu dapat komisi</Text>

                    <MyInput label='Nama Lengkap' value={data.nama_lengkap} placeholder="Masukan nama lengkap" onChangeText={x => setData({ ...data, nama_lengkap: x })} iconname='person' />
                    <MyGap jarak={20} />
                    <MyInput label='Username' value={data.username} placeholder="Masukan username" onChangeText={x => setData({ ...data, username: x })} iconname='person' />
                    <MyGap jarak={20} />
                    <MyInput label='Nomor Telepon' value={data.telepon} placeholder="Masukan nomor telepon" keyboardType='phone-pad' onChangeText={x => setData({ ...data, telepon: x })} iconname='call' />
                    <MyGap jarak={20} />
                    <MyInput label='Alamat' value={data.alamat} placeholder="Masukan alamat" onChangeText={x => setData({ ...data, alamat: x })} iconname='home' />
                    <MyGap jarak={20} />
